...
export default function PollManagement({ language = "hi", userRole = "admin" }: PollManagementProps) {
  ...
  const fetchAnalytics = async () => {
    try {
      const res = await fetch("/api/polls/analytics");
      const d = await res.json();
      if (d.success) {
  ...
  const handleDuplicate = async (pollId: string) => {
    try {
      setSubmitting(true);
      const res = await fetch(`/api/polls/${pollId}/duplicate`, { method: "POST" });
      const d = await res.json();
      if (d.success) {
  ...
  const handleArchive = async (pollId: string) => {
    try {
      setSubmitting(true);
      const res = await fetch(`/api/polls/${pollId}/archive`, { method: "POST" });
      const d = await res.json();
      if (d.success) {
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              ...
              <button
                ...
                className="px-3 py-1 bg-slate-100 text-xs font-bold rounded-xl"
              >
                Close
              </button>
            </div>
